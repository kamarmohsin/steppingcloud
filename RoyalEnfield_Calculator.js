...
var $mj = jQuery.noConflict();
	...
	$mj(document).ready(function(){
                              ...
                              var arrayOfElem = ['current_fixedCTC_an', 'current_totalFPA_an', 'current_totalBasic_fpa_an', 'current_totalVpay_an', 'current_totalRetBenifit_an', 'BasicSal_mon', 'housingAlMonthly', 'adjustmentHRA_mon', 'conveyance_mon', 'education_mon', 'medical_mon', 'city_mon', 'foodAllowance_mon', 'LTA_mon', 'current_grandTotal_an', 'current_basic_pm', 'current_specialAllowances_pm', 'current_personalAllowances_pm', 'current_FPA_pm', 'current_HRA_pm', 'current_conveyance_pm', 'current_GradeAllowance_pm', 'current_chilEducationAllow_pm','current_medicalAllow_pm','current_educationAllow_pm','current_OtherswithFCTC_pm','current_fixedCTC_pm','current_medicalReimburment_pm','current_educationReimbursment_pm','current_lta_pm','current_OtherswithFPA_pm','current_totalFPA_pm','current_totalBasic_fpa_pm','current_BonusExgratia_pm','current_pmpBonus_pm','current_totalVpay_pm','current_saf_pm','current_pf_pm','current_gratutity_pm','current_totalRetBenifit_pm','current_grandTotal_pm','RR_totalCompensation_an','RR_totalCompensation_pm','RR_fixedCTC_an','RR_fixedCTC_pm','RR_variablePay_an','RR_variablePay_pm','RR_fixedBasic_an','RR_fixedBasic_pm','RR_providentFund_an','RR_providentFund_pm','RR_gratuity_an','RR_gratuity_pm','RR_flexiBenefitpay_an','RR_flexiBenefitpay_pm','proposed_totalBasic_fpa_an','proposed_totalBasic_fpa_pm','proposed_totalRetBenifit_an','proposed_totalRetBenifit_pm','proposed_totalVariablePay_an','proposed_totalVariablePay_pm','proposed_grandTotal_an','proposed_grandTotal_pm','IncreasedPer_totalBasic_fpa_an','IncreasedPer_totalRetBenifit_an','IncreasedPer_totalVariablePay_an','IncreasedPer_grandTotal_an','percentile', 'totalCompensationInword','fixedCTCInword','pfInword','basicInword','fpaInword'];
			...
			for (var l=0; l < arrayOfElem.length; l++) {
				...
				var variablePer_grpManagerAbove = (isNaN(parseFloat(inputVariable)) ? 20 : parseFloat(inputVariable) );
				var variablePer_managerTeamManager = (isNaN(parseFloat(inputVariable)) ? 15 : parseFloat(inputVariable) );
				var variablePer_uptoDeputyManager = (isNaN(parseFloat(inputVariable)) ? 12 : parseFloat(inputVariable) );
				...
				CTCcalculation = {
						...
						var variablePay = {
							'grpManagerAbove': (inputfixedCTC * variablePer_grpManagerAbove)/100,
							'managerTeamManager': (inputfixedCTC * variablePer_managerTeamManager)/100,
							'uptoDeputyManager': (inputfixedCTC * variablePer_uptoDeputyManager)/100,
						}

						var fixedBasic ={
							'grpManagerAbove': ( inputfixedCTC * 25)/100,
							'managerTeamManager': ( inputfixedCTC * 25)/100,
							'uptoDeputyManager': ( inputfixedCTC * 25)/100,	
						}

						
						var gratuity ={

							'grpManagerAbove': Math.round(( fixedBasic['grpManagerAbove'] * 4.81)/100),
							'managerTeamManager': Math.round(( fixedBasic['grpManagerAbove'] * 4.81)/100),
							'uptoDeputyManager': Math.round(( fixedBasic['grpManagerAbove'] * 4.81)/100),
						}
